...
指定 `Dockerfile` 所在文件夹的路径， `Compose` 将会利用它自动构建这个镜像，然后使用这个镜像。这个路径可以是绝对路径或者相对 docker-compose.yml 文件的路径。
...
你也可以使用 `context` （上下文）指令指定 `Dockerfile` 所在文件夹的路径。使用 `dockerfile` 指令指定 `Dockerfile` 文件名。
