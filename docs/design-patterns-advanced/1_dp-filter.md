...
过滤器模式与[责任链模式](/design-patterns-base/7_dp-duty.html)的相似之处是处理过程都一环一环地进行，不同之处在于责任链中责任的传递一般会有一定的顺序，而过滤器通常没有这种顺序，所以**过滤器比责任链简单**。当然，过滤器也可以按照职责模式的方式来实现，这时我们认为每一次的过滤都是一种职责（一个任务），而整个过滤流程是一种特殊的链。
...
![过滤模式](./imgs/filter_mode_frame.png)
