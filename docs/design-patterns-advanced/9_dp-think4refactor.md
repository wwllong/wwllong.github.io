...
第一种情况属于项目本身代码的重构。这种重构应该贯穿于软件的开发过程中，只要你闻到代码的坏味道即可进行。我们可以遵循[三次法则](/design-patterns-advanced/8_dp-others-principles.html#rot原则)来进行重构。虽然重构可以随时随地进行，但还是需要一些触发事件。主要有以下几点：
