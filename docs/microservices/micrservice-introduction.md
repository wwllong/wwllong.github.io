...
以这种风格编写的应用是很常见的。他们**容易开发**，因为我们的 IDE 和其他工具就是专注于构建单体应用。这些应用程序也很**容易测试**， 您可以通过简单地启动并使用如 Selenium 测试包来测试 UI 以轻松地实现端到端 (end-to-end) 测试。单体应用同样**易于部署**。您只需拷贝打包好的应用程序到服务器上。您还可以通过运行多个副本和结合负载均衡器来扩展应用。在项目的早期阶段，它可以良好运作。
...
![microservices-app](./imgs/microservices-app.png)
...
![load-balancer](./imgs/load-balancer.png)
